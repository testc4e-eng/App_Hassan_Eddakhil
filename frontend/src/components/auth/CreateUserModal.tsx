...
export function CreateUserModal({ open, onOpenChange, onSubmit }: Props) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSubmit(form);
      onOpenChange(false);
      setForm({ full_name: "", email: "", password: "", role: "USER", status: "ACTIVE" });
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Ajoutez un compte ADMIN ou USER.</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="grid gap-4">
          <div className="grid gap-2">
            <Label htmlFor="create-full-name">Nom complet</Label>
            <Input id="create-full-name" value={form.full_name} onChange={(e) => setForm((prev) => ({ ...prev, full_name: e.target.value }))} required />
          </div>
          <div className="grid gap-2">
          ...
          <div className="grid gap-2">
            ...
            <Input id="create-password" type="password" value={form.password} onChange={(e) => setForm((prev) => ({ ...prev, password: e.target.value }))} required />
          </div>
