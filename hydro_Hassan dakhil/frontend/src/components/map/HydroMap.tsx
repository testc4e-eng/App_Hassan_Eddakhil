...
function AutoMapView({
  ...
  }, [dataSignature, resetViewRequest?.tick, layers, barrages, map]);
  ...
  useEffect(() => {
    ...
    const target = station ?? barrage ?? subBasin ?? basin;
    if (!target) return;
    ...
    if (geometryType.includes("Point")) {
      ...
      if (Array.isArray(point) && point.length >= 2) {
        map.flyTo([Number(point[1]), Number(point[0])], 12, { duration: 0.65 });
