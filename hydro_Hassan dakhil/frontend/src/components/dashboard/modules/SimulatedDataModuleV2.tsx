    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="max-h-80 overflow-auto rounded-md border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {availabilityFiltered.map((row) => (
                <tr key={`${row.data_type}-${row.entity_code}-${row.variable_code}-${row.run_id}`} className="border-t">
          ...
          {availabilityFiltered.length === 0 && <div className="p-3 text-sm text-muted-foreground">Aucun resultat.</div>}
        </div>
      </div>
