...
export function FilterBar({
  ...
  const visibleRows = useMemo(() => {
    const filtered = (rows as any[]).filter(
      (r) =>
        !HIDDEN_SCENARIO_CODES.has(String(r.scenario_code || "")) &&
  ...
  const stationList = useMemo(() => {
    return deduplicateSelectOptions(
      getStationsForModule(moduleCode, selectedRunId),
  ...
  const runOptions = useMemo(() => {
    if (moduleCode === "climat") {
      const observed = runs.find(
        (run) => run.is_observed && run.scenario_code === "OBSERVED"
      );
      return observed
        ? deduplicateSelectOptions(
            [
              {
                ...
                scenario_code: observed.scenario_code,
                scenario_name: t("filters.sourceObserved"),
    ...
    const map = new Map<string, HydroRunOption>();

    for (const scenario of NORMALIZED_SWAT_SCENARIOS) {
      const row = runs.find(
        (item) =>
          !item.is_observed &&
          item.scenario_code === scenario.code &&
          isNormalizedSwatScenarioCode(String(item.scenario_code || ""))
      );
    ...
    return deduplicateSelectOptions(
      Array.from(map.values()).sort((a, b) => {
        const rank = NORMALIZED_SWAT_SCENARIO_ORDER.get(
          a.scenario_code as any
        ) ?? 999;
        const other = NORMALIZED_SWAT_SCENARIO_ORDER.get(
          b.scenario_code as any
        ) ?? 999;
  ...
  const variableOptions = useMemo(() => {
    ...
    for (const r of visibleRows as any[]) {
      if (HIDDEN_SCENARIO_CODES.has(String(r.scenario_code || ""))) continue;
  ...
  const matchesSelectedVariable = useCallback(
    ...
    [selectedVarId, selectedVariableIdentity]
  );
  ...
  const selectedRows = useMemo(
    () =>
      (visibleRows as any[]).filter((r) => {
        if (!selectedStationId) return false;
        if (Number(r.station_id) !== selectedStationId) return false;

        if (isScenarioComparisonActive) {
          if (!compareRunIds.includes(Number(r.run_id))) return false;
        } else if (selectedRunId && Number(r.run_id) !== selectedRunId) {
          return false;
        }
