...
const EROSION_TIME_STEP_PRIORITY: Record<ErosionTimeStep, number> = {
  daily: 0,
  monthly: 1,
  annual: 2,
};
...
function timeStepRank(timeStep: string | null | undefined): number {
  if (!timeStep) return Number.POSITIVE_INFINITY;
  if (timeStep === "daily") return EROSION_TIME_STEP_PRIORITY.daily;
  if (timeStep === "monthly") return EROSION_TIME_STEP_PRIORITY.monthly;
  if (timeStep === "annual") return EROSION_TIME_STEP_PRIORITY.annual;
  return Number.POSITIVE_INFINITY;
}

function resolveStorageTimeStep(scenarioCode: string, catalogTimeStep: string): string {
  if (scenarioCode.startsWith("scenario_")) {
    return "yearly";
  }
  if (catalogTimeStep === "annual") {
    return "yearly";
  }
  return catalogTimeStep || "daily";
}
...
export class ErosionSwatSeriesService {
  ...
  private getErosionVariableDefs(): Promise<ErosionVariableDef[]> {
    if (!this.variableDefsCache) {
      this.variableDefsCache = Promise.resolve(EROSION_VARIABLE_DEFS);
    }
    return this.variableDefsCache;
  }

  private syntheticTsId(runId: number, stationId: number, propertyId: number): number {
    return runId * 1_000_000 + stationId * 10_000 + propertyId;
  }
  ...
  private async getSimulatedAvailabilityRowsFromCatalog(): Promise<ErosionAvailabilityRow[]> {
    ...
    const rows = await this.query<{
      ...
      ORDER BY v.station_id, v.run_id, v.property_id,
        ...
        v.ts_id
      `,
      [EROSION_SCENARIO_CODES, EROSION_VARIABLE_DEFS.map((def) => def.property_id)]
    );
    ...
    for (const row of rows) {
      ...
      availability.push({
        ts_id: this.syntheticTsId(Number(row.run_id), displayStation.station_id, def.property_id),
        module_code: "erosion",
        ...
        source_type: "simulated",
        time_step: (String(row.time_step) as ErosionTimeStep) || "daily",
        n_measures: nPoints,
        dt_min: row.dt_min,
        dt_max: row.dt_max,
        v_min: null,
  ...
  private async getSimulatedAvailabilityRowsFromMatView(): Promise<ErosionAvailabilityRow[]> {
    ...
    for (const row of rows) {
      ...
      const tsId = this.syntheticTsId(Number(row.run_id), displayStation.station_id, def.property_id);
      availability.push({
        ts_id: tsId,
        module_code: "erosion",
        station_id: displayStation.station_id,
  ...
  private async getSimulatedAvailabilityRows(): Promise<ErosionAvailabilityRow[]> {
    ...
    const promise = (async () => {
      if (await this.db.relationExists("public.v_ts_catalog_enriched")) {
        return this.getSimulatedAvailabilityRowsFromCatalog();
      }

      if (await this.db.relationExists("api.mv_hydro_station_stats")) {
        return this.getSimulatedAvailabilityRowsFromMatView();
      }
      ...
      for (const row of rows) {
        ...
        const def = defRows.get(Number(row.property_id));
        if (!def) continue;
        const tsId = this.syntheticTsId(Number(row.run_id), Number(row.station_id), def.property_id);
        const nPoints = toNumber(row.n_measures) ?? 0;
        availability.push({
          ts_id: tsId,
          module_code: "erosion",
  ...
  async getCatalog(stationId: number, runId: number): Promise<ErosionCatalogRow[]> {
    ...
    const fallbackRows = (await this.getAvailability({})).filter((row) => {
      if (row.run_id !== runId) return false;
  ...
  async getDateRange(stationId: number, runId: number, propertyId: number): Promise<ErosionDateRangeRow> {
    ...
    if (!row) {
      ...
      const suffix = String(subbasinId);
      const fallbackRow = (await this.getAvailability({ propertyId })).find(
        (item) =>
          item.run_id === runId &&
  ...
  async getBundle(params: {
    ...
    const bundleParts = await Promise.all(
      defs.map(async (def) => {
        const availabilityRow = catalogRows.find((row) => row.property_id === def.property_id);
  ...
  async getSubbasinAvailability(subbasinStationId?: number): Promise<ErosionSubbasinAvailabilityRow[]> {
    if (await this.db.relationExists("public.v_ts_catalog_enriched")) {
      return this.getSubbasinAvailabilityFromCatalog(subbasinStationId);
    }

    if (await this.db.relationExists("api.mv_hydro_station_stats")) {
      return this.getSubbasinAvailabilityFromMatView(subbasinStationId);
    }
  ...
  async getSubbasinSeries(
    ...
    const where: string[] = [
      "COALESCE(m.subbasin_id, s.sub_code) = $1",
      "mr.run_id = $2",
      "s.time_step = $3",
      "s.scenario_code = ANY($5::text[])",
    ...
    const sql = `
      SELECT
        date_trunc($4, s.period_date::timestamp)::date::text AS period,
