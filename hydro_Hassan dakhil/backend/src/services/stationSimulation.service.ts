...
export class StationSimulationService {
  ...
  private async resolveScenario(runId?: number): Promise<ScenarioRow> {
    ...
    return (
      latestAccessImport || {
        run_id: null,
        scenario_code: "SWAT_OUTPUT",
        scenario_name: "SWAT_OUTPUT",
        is_observed: false,
        import_id: null,
      }
    );
  ...
  private async resolveAccessScenario(preferredScenarioCode?: string): Promise<{
    scenarioCode: string;
    importId: number | null;
  }> {
    ...
    if (preferred) {
      const preferredRow = await this.db.queryOne<{ scenario_code: string; import_id: number }>(
        `
        SELECT scenario_code, import_id
        FROM access.rch_results
      ...
      if (preferredRow) {
        return {
          scenarioCode: preferredRow.scenario_code,
          importId: preferredRow.import_id,
        };
      }
    }

    const latest = await this.db.queryOne<{ scenario_code: string; import_id: number }>(
      `
      SELECT scenario_code, import_id
      FROM access.rch_results
      ORDER BY import_id DESC
    ...
    return {
      scenarioCode: latest?.scenario_code || preferred || "SWAT_OUTPUT",
      importId: latest?.import_id ?? null,
    };
  }
  ...
  private async loadSimulatedFlow(
    ...
    if (await this.db.relationExists("api.mv_hydro_station_timeseries")) {
      const params: unknown[] = [subbasinId, scenarioCode, "rch"];
      const where: string[] = [
        "sub_code = $1",
        "scenario_code = $2",
        "source_table = $3",
        "flow_out_cms IS NOT NULL",
      ];
  ...
  async getStationSimulations(params: GetStationSimulationParams): Promise<StationSimulationResponse> {
    ...
    const accessScenario = await this.resolveAccessScenario(params.scenarioCode || scenario.scenario_code);
    if (params.scenarioCode && accessScenario.scenarioCode !== params.scenarioCode) {
      warnings.push(
        `Scenario code ${params.scenarioCode} not found in access results. Falling back to ${accessScenario.scenarioCode}.`
      );
    }
    ...
    if (!observed.points.length) {
      warnings.push(`No observed STREAMFLOW series found for station_id=${params.stationId}.`);
    }

    if (!simulated.length) {
      warnings.push(
        `No simulated SWAT reach flow found for subbasin_id=${mapping.subbasin_id} and scenario_code=${accessScenario.scenarioCode}.`
      );
    }
    ...
    return {
      ...
      scenario: {
        ...
        scenarioCode: accessScenario.scenarioCode,
        scenarioName: scenario.scenario_name,
        importId: accessScenario.importId ?? scenario.import_id,
      },
      observed: {
        tsId: observed.tsId,
