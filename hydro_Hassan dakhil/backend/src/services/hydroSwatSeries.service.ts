...
export class HydroSwatSeriesService {
  ...
  private async getAvailabilityRowsFromMatView(): Promise<HydroAvailabilityRow[]> {
    ...
    for (const row of rows) {
      const key = `${row.station_id}:${row.run_id}:${row.property_id}`;
      ...
      allRows.push({
        ...
        source_type: "simulated",
        time_step: String(row.time_step) as HydroTimeStep,
        n_measures: nPoints,
        dt_min: row.dt_min,
        dt_max: row.dt_max,
  ...
  private async getAvailabilityRows(): Promise<HydroAvailabilityRow[]> {
    ...
    const promise = (async () => {
      if (await this.db.relationExists("api.mv_hydro_station_stats")) {
        return this.getAvailabilityRowsFromMatView();
      }
      ...
      const results = await Promise.all([
        this.query<AvailabilitySummaryRow>(
          ...
          WITH source AS (
            SELECT
              COALESCE(r.station_id, m.station_id) AS station_id,
              st.station_code,
            ...
            JOIN core.stations st
              ON st.station_id = COALESCE(r.station_id, m.station_id)
  ...
  private async loadSeries(
    ...
    const sourceAlias = def.entity_type === "rch" ? "r" : def.entity_type === "sub" ? "s" : "h";
