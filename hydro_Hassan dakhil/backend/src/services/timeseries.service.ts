...
function normalizeAggForRange(
  agg: AggInterval,
  startDate?: string,
  endDate?: string
): AggInterval {
  if (agg !== "day" || !startDate || !endDate) {
    return agg;
  }
...
export class TimeseriesService {
  ...
  private async getLegacyBundle(
    ...
    const entries = await Promise.all(
      catalog.map(async (ts) => {
        if (!ts.start_date || !ts.end_date) return null;
        const sql = `
          SELECT
        ...
        return [ts.ts_id, rows] as const;
      })
    );

    for (const entry of entries) {
      if (!entry) continue;
      const [tsId, rows] = entry;
      aggregated[tsId] = rows;
    }
  ...
  async getBundle(
    ...
    const effectiveAgg = normalizeAggForRange(agg, startDate, endDate);
    ...
    return this.getLegacyBundle(filter, effectiveAgg, startDate, endDate);
  }
