...
export class SolidYieldService {
  ...
  async resolveScenarioByRunId(runId: number): Promise<{ scenario_code: string; scenario_name: string } | null> {
    return this.db.queryOne<{ scenario_code: string; scenario_name: string }>(
      `
