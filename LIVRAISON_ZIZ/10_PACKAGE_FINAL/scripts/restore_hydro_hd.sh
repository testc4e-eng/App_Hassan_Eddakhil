...
printf 'Recreating target database from template0...\n'
dropdb --if-exists --force --host "${DB_HOST}" --port "${DB_PORT}" --username "${DB_USER}" "${TARGET_DB}"
createdb --template=template0 --host "${DB_HOST}" --port "${DB_PORT}" --username "${DB_USER}" "${TARGET_DB}"
